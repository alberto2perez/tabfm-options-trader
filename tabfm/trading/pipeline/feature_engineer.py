...
# Long-leg strike offsets per short: adjacent and 2-strikes-away
_LONG_LEG_OFFSETS = (1, 2)
...
def engineer_features(
  ...
  for opt_type in ("call", "put"):
    ...
    for _, short in short_legs.iterrows():
      ...
      # Multiple long-leg offsets give sizing both narrow and wide widths per
      # short — recovery mode's smaller slice can still fit the narrow one.
      if opt_type == "put":
        long_cands = same_expiry[same_expiry["strike"] < short["strike"]].sort_values("strike")
        longs = [long_cands.iloc[-off] for off in _LONG_LEG_OFFSETS if len(long_cands) >= off]
      else:
        long_cands = same_expiry[same_expiry["strike"] > short["strike"]].sort_values("strike")
        longs = [long_cands.iloc[off - 1] for off in _LONG_LEG_OFFSETS if len(long_cands) >= off]
